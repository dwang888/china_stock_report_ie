...
class HTMLParser(object):
    ...
    def parse_content(self, html_file_path):
        ...
        return paragraphs
    def parse_content_from_text(self, html_txt):
        """
        解析 HTML 中的段落文本
        按顺序返回多个 paragraph 构成一个数组，
        每个 paragraph 是一个 content 行构成的数组
                    ...
                    if is_head_two_rowspan and row_length > 2:
                        try:
                            ...
                            for col_idx in range(col_length):
                                head_row[col_idx] = table_dict[0][col_idx] + table_dict[1][col_idx]# merge 2 row as a new head
    ...
    def parse_table_to_2d_dict(table):
        rs_dict = {}
        row_index = 0
        is_head_two_rowspan, is_head = False, True# assume first row of table is head
        for tr in table.find_all('tr'):
            col_index, cur_col_index = 0, 0
            for td in tr.find_all('td'):
                rowspan = td.get('rowspan')#how many rows this element takes
                ...
                if is_head:
                    if rowspan > 1 or colspan > 1:# detect mutliple-row head; maybe caused by some html parser
